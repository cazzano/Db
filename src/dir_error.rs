...
impl DirectoryState {
    ...
    pub fn set_current_path<P: AsRef<Path>>(&mut self, path: P) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn verify_file_exists<P: AsRef<Path>>(&self, file_path: P) -> Result<PathBuf, Box<dyn std::error::Error>> {
        let path_ref = file_path.as_ref();
        let full_path = self.resolve_path(path_ref);
        let canonical_path = full_path.canonicalize().map_err(|_| {
            format!("File '{}' not found in current directory: {}", 
        ...
        if canonical_path.is_file() {
            ...
            Err(format!("Path '{}' exists but is not a file", canonical_path.display()).into())
        }
    }

    pub fn sync_with_navigator<P: AsRef<Path>>(&mut self, path: P) -> Result<(), Box<dyn std::error::Error>> {
        let path_ref = path.as_ref();
